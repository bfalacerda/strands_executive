...
class SimRobotPolicyExecutor():
    def __init__(self, use_ros, port, file_dir, file_name, gen_new_pol, use_time):
...
if __name__ == '__main__':
    ...
    if len(filtered_argv) == 7:
        ...
        mdp_executor = SimRobotPolicyExecutor(use_ros, int(port), file_dir, model_file, gen_new_pol, use_time)
        if use_ros:
            mdp_executor.main()
        else:
